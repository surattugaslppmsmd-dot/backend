...
function verifyJWT(req, res, next) {
    ...
    jwt.verify(token, JWT_SECRET, (err, decoded) => {
        if (err)
            return res.status(401).json({ message: "Token tidak valid/kadaluwarsa" });
        req.user = decoded;
        next();
    });
}
...
app.post("/api/forms/:formType", upload.single("pdfFile"), async (req, res) => {
    ...
    try {
        ...
        if (file_url)
            safeFormData["file_url"] = file_url;
        ...
        if (Array.isArray(anggota) && anggota.length > 0) {
            for (const a of anggota) {
                if (a?.name && a?.nidn) {
                    await pool.query(`INSERT INTO anggota_surat (surat_type, surat_id, nama, nidn)
               VALUES ($1,$2,$3,$4)`, [config.table, record.id, a.name, a.nidn]);
                }
            }
